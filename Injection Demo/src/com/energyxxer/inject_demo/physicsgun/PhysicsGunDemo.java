...
import static com.energyxxer.inject.InjectionBuffer.InjectionType.REPEAT;
        ...
        connection.getLogObserver().addLogListener(l -> {
            ...
            if(stage == 1) {
                if(playerInfo.containsKey(username)) {
                    ...
                    connection.inject(IMPULSE, "execute " + username + " ~ ~ ~ scoreboard players tag @e[tag=pg_control_" + username + "$0,c=1] add pg_control_" + username);
                    //Set player's state to 2 (if entity found)
                    connection.inject(IMPULSE, "execute @e[tag=pg_control_" + username + "] ~ ~ ~ scoreboard players set " + username + " pg_state 2");
                    //Setting entity to have no gravity (if entity found)
                    connection.inject(IMPULSE, "entitydata @e[tag=pg_control_" + username + "] {NoGravity:1b}");
                    //Setting up entity's position getter
                    connection.inject(IMPULSE, "execute @e[tag=pg_control_" + username + "] ~ ~ ~ summon area_effect_cloud ~ ~ ~ {CustomName:\"$pgControlTransform#" + username + "\",Duration:2}");
                    connection.inject(IMPULSE, "execute @e[tag=pg_control_" + username + "] ~ ~ ~ teleport @e[type=area_effect_cloud,name=$pgControlTransform#" + username + "] ~ ~ ~ ~ ~");
                    //Commands from this point on will see their output in the logs
                    connection.inject(IMPULSE, "gamerule logAdminCommands true");
                    ...
                    connection.inject(IMPULSE, new Command("$pgEnable:" + username, "scoreboard players tag @e[tag=pg_control_" + username + "$0] remove pg_control_" + username + "$0"));
                    //Get the controlled entity's position
                    connection.inject(IMPULSE, new Command("pg_control_" + username, "entitydata @e[type=area_effect_cloud,name=$pgControlTransform#" + username + "] {pg:\"transform2\"}"));
                    //Set player's state back to 0 if no entities were found
                    connection.inject(IMPULSE, new Command("$pgDisable:" + username, "scoreboard players set @a[name=" + username + ",score_pg_state_min=1,score_pg_state=1] pg_state 0"));
                    //Disable command logging
                    connection.inject(IMPULSE, "gamerule logAdminCommands false");
                } else {
                ...
                if(playerInfo.containsKey(username)) {
                    ...
                    connection.inject(IMPULSE, "entitydata @e[tag=pg_control_" + username + "] {NoGravity:0b}");
                    connection.inject(IMPULSE, "execute @a[name=" + username + ",tag=!pg_sneaking] ~ ~ ~ entitydata @e[tag=pg_control_" + username + "] {Motion:[" + forward.x + "," + forward.y + "," + forward.z + "]}");
                    connection.inject(IMPULSE, "execute @a[name=" + username + ",tag=!pg_sneaking] ~ ~ ~ execute @e[tag=pg_control_" + username + "] ~ ~ ~ particle largesmoke ~ ~0.5 ~ 0 0 0 " + (MAX_REACH / player.distance) + " " + (int) (2 * (MAX_REACH - player.distance)) + " force");
                    connection.inject(IMPULSE, "scoreboard players set " + username + " pg_state 0");
                    connection.inject(IMPULSE, "scoreboard players tag @e[tag=pg_control_" + username + "] remove pg_control_" + username);
                } else {
                    playerInfo.put(username, new PGPlayerInfo(username));
                    connection.inject(IMPULSE, "tellraw @a {\"text\":\"[WARNING]: Player '" + username + "' requested an action but wasn't found on the player database.\",\"color\":\"yellow\"}");
                }
